...
##### Added by Ngaiman Chow for Knowledge Graph
import csv, sys
...
input_data = read_tsv('.......csv')

new_input_data = {}

# Iterate over the original list
for item in input_data:
    # Use the 'id' as the key and a dictionary of 'title' and 'text' as the value
    new_input_data[item['id']] = {"title": item['title'], "text": item['text']}



import hydra
from omegaconf import DictConfig, OmegaConf
@hydra.load_data(config_path="conf", config_name="dense_retriever")
...
def calculate_matches(
    ...
    ##### Added by Ngaiman Chow for Knowledge Graph
    from wikiextractor import calculate_question_passage_relevance
    import numpy
    new_closest_doc=[]
    for i, question_text in enumerate(questions_text):
        doc_index = []
        doc_score = []
        for j, retrieved_doc in enumerate(closest_docs[i][0]):
            if( wikiextractor.calculate_question_passage_relevance( question_text , new_input_data[int(retrieved_doc[5:])]['text'] ) ):
                doc_index.append( retrieved_doc )
                doc_score.append( closest_docs[i][1][j] )
        new_closest_doc.append( ( doc_index , numpy.array( doc_score ) ) )
    ##### Added by Ngaiman Chow for Knowledge Graph
