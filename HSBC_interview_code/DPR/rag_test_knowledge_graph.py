...
def ask_openai_to_judge_if_answer_correct_or_not(question,true_answers,reply):
    ...
    Does his/her reply can be regarded as correct or not? Please be honest and just answer one word: "correct" or "incorrect".
    """
    response = openai.ChatCompletion.create(
        model="gpt-3.5-turbo",
        messages = [ # Change the prompt parameter to the messages parameter
...
from question_relevancy import calculate_question_passage_relevance_abs
...
def main(cfg: DictConfig):
    ...
    for i, question in enumerate(questions):
        ...
        for passage_for_knowledge_graph_filter_id in closest_docs[i][0][:top_k]:
            passage_to_be_tested = input_passage_csv_key_by_id[passage_for_knowledge_graph_filter_id[5:]]['text']
            if calculate_question_passage_relevance_abs(question,passage_to_be_tested, 0.1):
                context.append(passage_to_be_tested)
