...
def generate_question_vectors(
    ...
    with torch.no_grad():
        for j, batch_start in enumerate(range(0, n, bsz)):
            ...
            #################### DEBUG ####################
            '''
            batch_tensors_concat = torch.stack(batch_tensors, dim=0)
            batch_tensors_numpy_array = batch_tensors_concat.numpy()
            all( batch_tensors_numpy_array[: , -1] == 102 )
            '''
            #################### DEBUG ####################
            ##### Modified by Ngaiman Chow on 2023-11-5 for solving the error:
